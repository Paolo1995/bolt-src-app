...
public final class PoolChunkList<T> implements PoolChunkListMetric {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public PoolChunkList(PoolArena<T> poolArena, PoolChunkList<T> poolChunkList, int i8, int i9, int i10) {
        this.arena = poolArena;
        this.nextList = poolChunkList;
        this.minUsage = i8;
        this.maxUsage = i9;
        this.maxCapacity = calculateMaxCapacity(i8, i10);
    }

    private static int calculateMaxCapacity(int i8, int i9) {
        int minUsage0 = minUsage0(i8);
        if (minUsage0 == 100) {
            return 0;
        }
        return (int) ((i9 * (100 - minUsage0)) / 100);
    }
    ...
    public boolean allocate(PooledByteBuf<T> pooledByteBuf, int i8, int i9) {
        ...
        for (PoolChunk<T> poolChunk = this.head; poolChunk != null; poolChunk = poolChunk.next) {
            if (poolChunk.allocate(pooledByteBuf, i8, i9)) {
                if (poolChunk.usage() >= this.maxUsage) {
                    remove(poolChunk);
                    this.nextList.add(poolChunk);
                    return true;
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public boolean free(PoolChunk<T> poolChunk, long j8, ByteBuffer byteBuffer) {
        poolChunk.free(j8, byteBuffer);
        if (poolChunk.usage() < this.minUsage) {
            remove(poolChunk);
            return move0(poolChunk);
        }
