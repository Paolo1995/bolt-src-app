...
public abstract class AbstractCoalescingBufferQueue {
    ...
    public AbstractCoalescingBufferQueue(Channel channel, int i8) {
        PendingBytesTracker newTracker;
        this.bufAndListenerPairs = new ArrayDeque<>(i8);
        if (channel == null) {
            newTracker = null;
        } else {
            newTracker = PendingBytesTracker.newTracker(channel);
        }
        this.tracker = newTracker;
    }

    private void decrementReadableBytes(int i8) {
        this.readableBytes -= i8;
        PendingBytesTracker pendingBytesTracker = this.tracker;
        if (pendingBytesTracker != null) {
            pendingBytesTracker.decrementPendingOutboundBytes(i8);
        }
    }

    private void incrementReadableBytes(int i8) {
        int i9 = this.readableBytes;
        int i10 = i9 + i8;
        if (i10 >= i9) {
            this.readableBytes = i10;
            PendingBytesTracker pendingBytesTracker = this.tracker;
            if (pendingBytesTracker != null) {
                pendingBytesTracker.incrementPendingOutboundBytes(i8);
                return;
            }
            return;
        }
        throw new IllegalStateException("buffer queue length overflow: " + this.readableBytes + " + " + i8);
    }
